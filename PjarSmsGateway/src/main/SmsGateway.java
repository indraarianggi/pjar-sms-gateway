...
//        SerialModemGateway gateway = new SerialModemGateway("modem.com4", "COM4", 9600, "", "");
    ...
    public void sendMessage(ArrayList no_tlp, ArrayList pesanutama, String pesantambahan) throws Exception {
        OutboundNotification outboundNotification = new OutboundNotification();
        System.out.println("Example: Send message from a serial gsm modem.");
        System.out.println(Library.getLibraryDescription());
        System.out.println("Version: " + Library.getLibraryVersion());
//        SerialModemGateway gateway = new SerialModemGateway("modem.com4", "COM4", 9600, "", "");
        ...
        for(int i=0;i<no_tlp.size();i++){
            String pesangambungan = pesanutama.get(i).toString() + "\n" + pesantambahan;
            OutboundMessage msg = new OutboundMessage(no_tlp.get(i).toString(),config.getConfig().get(key.header)+pesangambungan);
            Service.getInstance().sendMessage(msg);
        }
